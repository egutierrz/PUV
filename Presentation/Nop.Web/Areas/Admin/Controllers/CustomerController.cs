...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual async Task<string> ValidateCustomerRolesAsync(IList<CustomerRole> customerRoles, IList<CustomerRole> existingCustomerRoles)
        {
            ...
            if (rolesToAdd.Any(role => role.SystemName != NopCustomerDefaults.AccessRoleName) || rolesToDelete.Any())
            {
            ...
            //var isInGuestsRole = customerRoles.FirstOrDefault(cr => cr.SystemName == NopCustomerDefaults.BackgroundRoleName) != null;
            //var isInRegisteredRole = customerRoles.FirstOrDefault(cr => cr.SystemName == NopCustomerDefaults.AccessRoleName) != null;
        ...
        {
            var customers = await _customerService.GetAllCustomersAsync(customerRoleIds: new[] { (await _customerService.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.SuperAdminRoleName)).Id });
        ...
        public virtual async Task<IActionResult> Create(CustomerModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (newCustomerRoles.Any() && newCustomerRoles.FirstOrDefault(c => c.SystemName == NopCustomerDefaults.AccessRoleName) != null &&
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var customerRole in newCustomerRoles)
                {
                    ...
                    if (customerRole.SystemName == NopCustomerDefaults.SuperAdminRoleName && !await _customerService.IsAdminAsync(await _workContext.GetCurrentCustomerAsync()))
                        continue;
                ...
                //activity log
                await _customerActivityService.InsertActivityAsync(ActivityLogTypeEnum.PB_UserCreation.ToString(),
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (var customerRole in allCustomerRoles)
                    {
                        ...
                        if (customerRole.SystemName == NopCustomerDefaults.SuperAdminRoleName &&
                        ...
                        else
                        {
                            ...
                            if (customerRole.SystemName == NopCustomerDefaults.SuperAdminRoleName && !await SecondAdminAccountExistsAsync(customer))
                            {
        ...
        public virtual async Task<IActionResult> LoadCustomerStatistics(string period)
        {
            ...
            var searchCustomerRoleIds = new[] { (await _customerService.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.AccessRoleName)).Id };
        ...
        public virtual async Task<IActionResult> GdprExport(int id)
        {
            ...
            try
            {
                ...
                var bytes = new byte[0];//await _exportManager.ExportCustomerGdprInfoToXlsxAsync(customer, store.Id);
        ...
        public virtual async Task<IActionResult> ExportExcelAll(CustomerSearchModel model)
        {
            ...
            try
            {
                var bytes = new byte[0];//await _exportManager.ExportCustomersToXlsxAsync(customers);

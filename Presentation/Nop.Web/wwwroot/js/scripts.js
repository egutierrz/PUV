!(function (NioApp, $) {
  ...
  NioApp.Dropzone.init = function () {
    let parentDirectoryId = $("#ParentFolderId").val();
      NioApp.Dropzone('.upload-zone', {
        url: "/Admin/InformationRepository/AddFile?folderId=" + parentDirectoryId,
        addRemoveLinks: true,
        ...
        init: function () {
          ...
          this.on("removedfile", function (file) {
            console.log(file);
            let x = confirm('\u00BFDesea eliminar el archivo?');
            if (!x) return false;
            else {
              $.ajax({
                cache: false,
                type: "POST",
                url: "/Admin/InformationRepository/RemoveFile",
                data: {idFile:file.id},

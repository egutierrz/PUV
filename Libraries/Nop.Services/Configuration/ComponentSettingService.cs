...
namespace Nop.Services.Configuration
{
    ...
    /// Setting manager
    /// </summary>
    public partial class ComponentSettingService : IComponentSettingService
    {
        #region Fields

        private readonly IRepository<ComponentSetting> _settingRepository;
        ...
        public ComponentSettingService(IRepository<ComponentSetting> settingRepository,
        ...
        /// </returns>
        protected virtual async Task<IDictionary<string, IList<ComponentSetting>>> GetAllSettingsDictionaryAsync()
        {
            return await _staticCacheManager.GetAsync(NopSettingsDefaults.SettingsAllAsDictionaryCacheKey, async () =>
            {
                var settings = await GetAllSettingsAsync();

                var dictionary = new Dictionary<string, IList<ComponentSetting>>();
                foreach (var s in settings)
                {
                    var resourceName = s.Name.ToLowerInvariant();
                    var settingForCaching = new ComponentSetting
                    {
                        Id = s.Id,
                        Name = s.Name,
                        Value = s.Value,
                    ...
                    if (!dictionary.ContainsKey(resourceName))
                        //first setting
                        dictionary.Add(resourceName, new List<ComponentSetting>
                        {
                            settingForCaching
                        });
                    else
        ...
        /// <param name="clearCache">A value indicating whether to clear cache after setting update</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task InsertSettingAsync(ComponentSetting setting, bool clearCache = true)
        {
        ...
        /// <param name="clearCache">A value indicating whether to clear cache after setting update</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task UpdateSettingAsync(ComponentSetting setting, bool clearCache = true)
        {
            if (setting == null)
        ...
        /// <param name="setting">Setting</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task DeleteSettingAsync(ComponentSetting setting)
        {
        ...
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task DeleteSettingsAsync(IList<ComponentSetting> settings)
        {
        ...
        /// </returns>
        public virtual async Task<ComponentSetting> GetSettingByIdAsync(int settingId)
        {
        ...
        /// The task result contains the setting
        /// </returns>
        public virtual async Task<ComponentSetting> GetSettingAsync(string key, int storeId = 0, bool loadSharedValueIfNotFound = false)
        {
        ...
        /// The task result contains the settings
        /// </returns>
        public virtual async Task<IList<ComponentSetting>> GetAllSettingsAsync()
        {
        ...
        {
            var settingsToDelete = new List<ComponentSetting>();
        ...
        {
            await _staticCacheManager.RemoveByPrefixAsync(NopEntityCacheDefaults<ComponentSetting>.Prefix);
        }
